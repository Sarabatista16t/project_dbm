...
static all (callback){
    database.where("SELECT * FROM Social_Media", [], Social_Media, callback)
}
 
static get(id, callback){
    database.where("SELECT * FROM Social_Media WHERE id = ?", [id], Social_Media, callback)
}
...
save(callback){
    ...
    if(this.id){
        console.log("IF")
        database.run("UPDATE Social_Media SET  name =? ,  base_link =? ,  icon =?   WHERE id = ?", [ this.name, this.base_link, this.icon, this.id], callback)
    }else{
                console.log("else")

        database.run("INSERT INTO Social_Media ( name,  base_link,  icon ) values (?,?,?)", [ this.name ,  this.base_link ,  this.icon  ]) 
    }
} 
