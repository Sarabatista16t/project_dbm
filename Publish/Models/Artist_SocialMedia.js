...
static all (callback){
    database.where("SELECT * FROM Artist_SocialMedia", [], Artist_SocialMedia, callback)
}
 
static get(id, callback){
    database.where("SELECT * FROM Artist_SocialMedia WHERE id = ?", [id], Artist_SocialMedia, callback)
}
...
save(callback){
    ...
    if(this.id){
        console.log("IF")
        database.run("UPDATE Artist_SocialMedia SET  id_artist =? ,  id_social_media =? ,  link =?   WHERE id = ?", [ this.id_artist, this.id_social_media, this.link, this.id], callback)
    }else{
                console.log("else")

        database.run("INSERT INTO Artist_SocialMedia ( id_artist,  id_social_media,  link ) values (?,?,?)", [ this.id_artist ,  this.id_social_media ,  this.link  ]) 
    }
} 
