...
save(callback){
    ...
    if(this.id){
        console.log("IF")
        database.run("UPDATE Album SET  title =? ,  description =? ,  image =? ,  release_date =? ,  id_artist =? ,  id_producer =? ,  id_record_label =?   WHERE id = ?", [ this.title, this.description, this.image, this.release_date, this.id_artist, this.id_producer, this.id_record_label, this.id], callback)
    }else{
                console.log("else")

        database.run("INSERT INTO Album ( title,  description,  image,  release_date,  id_artist,  id_producer,  id_record_label ) values (?,?,?,?,?,?,?)", [ this.title ,  this.description ,  this.image ,  this.release_date ,  this.id_artist ,  this.id_producer ,  this.id_record_label  ]) 
    }
} 
